 ...
 */
const validateConfigData = (configData: ConfigData, options?: ConfigDataOptions) => {
 ...
 */
export const edit = async (oldName: string, configData: ConfigData): Promise<IServiceConfig | null> => {
    const config = await database.serviceConfig.get(oldName);
    ...
    const newConfigs = configData.filePath ? getConfigsFromFile(configData.filePath) : null;
