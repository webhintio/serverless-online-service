...
const loadScript = (context: QueueTestContext) => {
    return proxyquire('../../src/queue/queue', {
        '../misc': context.misc,
...
test('Constructor should create the instance of azure service bus', (t) => {
    ...
    new Queue('QueueName', 'connectionString'); // eslint-disable-line no-new
...
test('getMessagesCount should return the number of active messages in the queue', async (t) => {
    const queueResult = { MessageCount: '15' };
    ...
    t.is(result, parseInt(queueResult.MessageCount, 10));
});
