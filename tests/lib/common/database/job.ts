...
test('job.getStatusCount should return the number of jobs with that status', async (t) => {
    const sandbox = t.context.sandbox;
...
test('job.getStatusCount with a since parameter should return the number of jobs since that date with that status', async (t) => {
    const sandbox = t.context.sandbox;
    const queryExecStub = sandbox.stub(t.context.query, 'exec').resolves();
...
test('job.getCount should return the number of jobs in the database', async (t) => {
    const sandbox = t.context.sandbox;
    const queryExecStub = sandbox.stub(t.context.query, 'exec').resolves();
    const job = loadScript(t.context);
...
test('job.getCount with a since parameter should return the number of jobs since that date', async (t) => {
    const sandbox = t.context.sandbox;
    const queryExecStub = sandbox.stub(t.context.query, 'exec').resolves();
