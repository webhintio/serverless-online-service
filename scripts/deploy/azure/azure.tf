...
resource "azurerm_function_app" "functionsServices" {
    ...
    app_settings = {
        ...
        "DatabaseConnection" = "${replace(azurerm_cosmosdb_account.cosmosdb.connection_strings[0], "/?", "/${azurerm_cosmosdb_mongo_database.mongodb.name}?")}"
